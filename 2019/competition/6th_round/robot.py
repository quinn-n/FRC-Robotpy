...
class Robot(wpilib.TimedRobot):
    ...
    def robotInit(self):
        ...
        self.db = NetworkTables.getTable("SmartDashboard/DB")
...
#        self.start_cameras()
    ...
    def robotPeriodic(self):
        ...
        compressor_amps = self.compressor.getCompressorCurrent()
        self.db.putBoolean(COMPRESSOR_ACTIVE_ENTRY, pressure_switch)
        self.db.putNumber(COMPRESSOR_AMPS_ENTRY, compressor_amps)
    ...
    def teleopPeriodic(self):
        ...
        #Beam motor
        #Auto lift beam if lift is retracted.
        if self.ds.getStickButton(STICK, BEAM_UP_BTN) and not self.retracted_lift:
            self.beam_motor.set(BEAM_SPEED)
        elif self.ds.getStickButton(STICK, BEAM_DOWN_BTN) or self.retracted_lift:
            self.beam_motor.set(-BEAM_SPEED)
        else:
