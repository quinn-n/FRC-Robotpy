...
class Robot(wpilib.TimedRobot):
    ...
    def robotInit(self):
        ...
        self.lift_motor = ctre.wpi_talonsrx.WPI_TalonSRX(LIFT_MOTOR)
        self.lift_motor.setInverted(LIFT_INV)
        self.intake = ctre.wpi_talonsrx.WPI_TalonSRX(INTAKE_MOTOR)
        self.intake.setInverted(INTAKE_INV)
        #Setup solenoids
...
#        self.start_cameras()
    ...
    def teleopPeriodic(self):
        ...
        #lift pneumotic if lift isn't retracted
        if self.ds.getStickButton(STICK, LIFT_EXT_BTN) or not self.retracted_lift:
            self.beam_solenoid.extend()
        elif self.ds.getStickButton(STICK, LIFT_RET_BTN) and not self.retracted_lift:
            self.beam_solenoid.retract()
        else:
            self.beam_solenoid.stop()
        #Retract and extend the intake 
        if self.ds.getStickButton(STICK, RETRACT_BUTTON):
            self.retracted_lift = True
        elif self.ds.getStickButton(STICK, EXTEND_BUTTON):
            self.retracted_lift = False
