...
class Robot(wpilib.TimedRobot):
    ...
    def teleopPeriodic(self):
        ...
        for_neg = neg(forward_speed)
        rot_neg = neg(rotate_speed)
        forward_speed = FOR_PARABOLA(forward_speed)
        rotate_speed = ROT_PARABOLA(rotate_speed)

        #Make sure values that were negative before are negative now (eg. a curve of x^2)
        if neg(forward_speed) != for_neg:
            forward_speed *= -1
        if neg(rotate_speed) != rot_neg:
            rotate_speed *= -1
