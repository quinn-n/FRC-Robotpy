...
class robot(wpilib.IterativeRobot):
    ...
    def teleopInit(self):
        ...
        print("Encoder.getRate(): "+str(self.encoder.getRate()))
        self.oldEncoderRevolutions = self.encoder.get()
    def teleopPeriodic(self):
        if self.oldEncoderSpeed != self.encoder.getRate():
            self.oldEncoderSpeed = self.encoder.getRate()
            print("Encoder speed: "+str(self.oldEncoderSpeed))
        if self.encoder.get() != self.oldEncoderRevolutions:
            self.oldEncoderRevolutions = self.encoder.get()
            print("Encoder revolutions: "+str(self.oldEncoderRevolutions))
        ...
        #encoder stuffs
        if self.driverStation.getStickButton(joystick,resetEncoderButton):
            self.encoder.reset()
        if self.encoder.get() > 1500 or self.encoder.get() < -1500:
            turningSpeed = 0
            sideSpeed = 0
            forwardSpeed = 0
