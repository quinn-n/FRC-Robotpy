...
class robot(wpilib.IterativeRobot):
    ...
    def driveForwards(self,distance,speed=.25):
        ...
        self.oldEncoderValue = self.encoder.getPulseWidthPosition()
        while (self.encoder.getPulseWidthPosition()-self.oldEncoderValue)*self.driveWheelCircumference < distance:
            time.sleep(.05)
    ...
    def driveBackwards(self,distance,speed=-.25):
        ...
        self.setRightMotorSpeed(speed)
        self.oldEncoderValue = self.encoder.getPulseWidthPosition()
        while (self.encoder.getPulseWidthPosition()-self.oldEncoderValue)*self.driveWheelCircumference/-1 < distance:
            time.sleep(.05)
        self.setLeftMotorSpeed(0)
        self.setRightMotorSpeed(0)
    def robotInit(self):
        ...
        self.encoder = ctre.sensorcollection.SensorCollection(self.rearRightDriveMotor)
...
#            self.setRightMotorSpeed(0)
    ...
    def teleopPeriodic(self):
        self.newEncoderValue = self.encoder.getPulseWidthPosition()
