...
class robot(wpilib.IterativeRobot):
    ...
    def driveForwards(self,distance,speed=.25):
        #self.frontLeftDriveMotor.ControlMode = 1
        self.frontLeftDriveMotor.set(speed,mode=1)
        """
        self.setLeftMotorSpeed(speed)
        self.setRightMotorSpeed(speed)
        self.oldEncoderValue = self.encoder.getPinStateQuadA()
        while (self.encoder.getPinStateQuadA()-self.oldEncoderValue)*self.driveWheelCircumference < distance:
            time.sleep(.05)
        self.setLeftMotorSpeed(0)
        self.setRightMotorSpeed(0)
        """
    def driveBackwards(self,distance,speed=-.25):
        self.setLeftMotorSpeed(speed)
        self.setRightMotorSpeed(speed)
        self.oldEncoderValue = self.encoder.getPinStateQuadA()
        while (self.encoder.getPinStateQuadA()-self.oldEncoderValue)*self.driveWheelCircumference/-1 < distance:
            time.sleep(.05)
        self.setLeftMotorSpeed(0)
        self.setRightMotorSpeed(0)
    def robotInit(self):
        ...
        self.frontLeftDriveMotor.FeedbackDevice = 0
        self.rearLeftDriveMotor.FeedbackDevice = 0
        self.frontRightDriveMotor.FeedbackDevice = 0
        self.rearRightDriveMotor.FeedbackDevice = 0
        ...
        self.driveWheelCircumference = driveWheelDiameter*pi
        #cameras
...
#            self.setRightMotorSpeed(0)
    ...
    def teleopInit(self):
        self.oldEncoderValue = self.frontLeftDriveMotor.getAnalogInVel()
        print("Got encoder value: "+str(self.oldEncoderValue))
    def teleopPeriodic(self):
        self.newEncoderValue = self.frontLeftDriveMotor.getAnalogInVel()
        """
        if self.newEncoderValue != self.oldEncoderValue:
            print("Got new encoder value: "+str(self.newEncoderValue))
            self.oldEncoderValue = self.newEncoderValue
        """
        print("Got encoder value: "+str(self.newEncoderValue))
